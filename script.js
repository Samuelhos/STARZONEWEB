...
function generateSchedules(){
  ...
  for(let i = 0; i < 30; i++){
    ...
    const data = bookings[formatted] || { 
      nintendo:{ status:"Ready", jam:"10:00 - 22:00" },
      playbox:{ status:"Ready", jam:"10:00 - 22:00" }
    };
  ...
  <p>
    Nintendo OLED :
    <span class="${data.nintendo.status === "Ready" ? "available" : "booked"}">
      ${data.nintendo.status}
    </span>
    <br>
    <small>Jam tersedia : ${data.nintendo.jam}</small>
  </p>

  <p>
    Playbox :
    <span class="${data.playbox.status === "Ready" ? "available" : "booked"}">
      ${data.playbox.status}
    </span>
    <br>
    <small>Jam tersedia : ${data.playbox.jam}</small>
  </p>
`;
...
function scheduleMatchesFilter(index, data){
  const nintendoReady = data.nintendo.status === "Ready";
  const playboxReady = data.playbox.status === "Ready";
  const anyBooked = data.nintendo.status === "Booked" || data.playbox.status === "Booked";
...
function generateAdminSchedule(){
  ...
  for(let i = 0; i < 30; i++){
    ...
    <input 
      type="text"
      value="${bookings[formatted].nintendo.jam}"
      placeholder="08:00 - 12:00"
      onchange="updateJam('${formatted}','nintendo', this.value)"
    >
  
    <button 
      class="toggle-btn ${bookings[formatted].nintendo.status === "Ready" ? "ready" : "booked"}"
      onclick="toggleStatus('${formatted}','nintendo')"
    >
      ${bookings[formatted].nintendo.status}
    </button>
    ...
    <input 
      type="text"
      value="${bookings[formatted].playbox.jam}"
      placeholder="13:00 - 22:00"
      onchange="updateJam('${formatted}','playbox', this.value)"
    >
  
    <button 
      class="toggle-btn ${bookings[formatted].playbox.status === "Ready" ? "ready" : "booked"}"
      onclick="toggleStatus('${formatted}','playbox')"
    >
      ${bookings[formatted].playbox.status}
    </button>
  `;
...
function toggleStatus(date, product){
  ...
  bookings[date][product].status =
    bookings[date][product].status === "Ready"
      ? "Booked"
      : "Ready";
...
function updateJam(date, product, value){
  ...
  bookings[date][product].jam = value;

  set(bookingsRef, bookings)
    .catch(() => alert("Gagal update jam."));
}
